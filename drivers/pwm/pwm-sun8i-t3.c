		...
		struct pwm_state *state)
{
	...
	if (chip->date->has_prescaler_bypass) {
		/* set pwm output bypass */
		if (ch % 2)
...
static int sunxi_pwm_apply(struct pwm_chip *chip, struct pwm_device *pwm,
		struct pwm_state *state)
{
	/* member chip ---> struct sunxi_pwm_chip */
	...
	if ((cstate.period != state->period) ||
		...
		ret = sunxi_pwm_config(sunxi_pwm, ch, &cstate);
		if (ret) {
			dev_err(chip->dev, "failed to enable PWM clock\n");
			return ret;
		}
...
static const struct sunxi_pwm_data sunxi_pwm_data_t3 = {
...
static const struct of_device_id sunxi_pwm_dt_ids[] = {
	{
		.compatible = "allwinner,sun8i-t3-pwm",
	},
};
...
MODULE_ALIAS("platform:sun8i-t3-pwm");
MODULE_AUTHOR("Hao Zhang <hao5781286@gmail.com>");
MODULE_DESCRIPTION("Allwinner sun8i-t3 PWM driver");
